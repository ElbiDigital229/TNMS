      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            {canCreate && (<button onClick={() => navigate("/acquisitions/buildings/new")} className={cls.btnPrimary}><Plus size={14} /> Create Building</button>)}
          </>
        }
      />
      ...
      <div className="space-y-2 md:hidden">
        {loading ? (
          ...
          rows.map((r) => (
            <div
              key={r.id}
              onClick={() => navigate(`/acquisitions/buildings/${r.id}/edit`)}
              className="rounded-lg bg-white p-3 ring-1 ring-gray-200 active:bg-gray-50 transition-colors cursor-pointer"
            >
              <div className="flex items-center justify-between">
                <span className={cls.mono + " text-[12px]"}>{r.buildingCode}</span>
                <div onClick={(e) => e.stopPropagation()}>
                  <RowActionsMenu
                    items={buildBuildingMenuItems({ row: r, canEdit, canDelete, navigate, onArchive: () => setConfirmArchive(r), onRestore: () => handleRestore(r) })}
                  />
                </div>
              </div>
              {r.agent && (
              ...
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                ...
                {r.buildingStatus && <span className="inline-flex rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-700">{BUILDING_STATUS_LABELS[r.buildingStatus]}</span>}
              ...
              <div className="mt-2 flex items-center justify-between text-[11px] text-gray-500">
                <span>
                  {r.coveredAreaSqft ? `${r.coveredAreaSqft} sqft` : ""}
                  {r.floors != null ? `${r.coveredAreaSqft ? " · " : ""}${r.floors} floors` : ""}
                </span>
                <span className="font-semibold text-gray-900">{fmtPKR(r.askingRent)}</span>
              </div>
            </div>
          ))
        )}
          ...
          <table className={cls.table}>
            ...
            <tbody>
              {loading ? (
                ...
                rows.map((r) => (
                  <tr key={r.id} onClick={() => navigate(`/acquisitions/buildings/${r.id}/edit`)} className={cls.trClick}>
