...
export default function AcquisitionBuildingFormPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (!isEdit) return;
    acquisitionBuildingApi
      .getById(id!)
      .then((res) => {
        const r = res.data.data;
        setBuildingCode(r.buildingCode);
        setForm({
          agentId: r.agentId || "",
          city: r.city,
      ...
      .catch(() => toast.error("Failed to load building record"))
  ...
  return (
    <div>
      <button
        onClick={() => navigate("/acquisitions/buildings")}
      ...
      <PageHeader title={isEdit ? `Edit Building — ${buildingCode}` : "Create Building"} />
      ...
      <div className={`mx-auto max-w-2xl ${cls.card} p-4`}>
        <div className="space-y-3">
          ...
          <div>
            ...
            <input type="text" placeholder="Search by code, name, or company…" value={agentSearch} onChange={(e) => setAgentSearch(e.target.value)} className={`${cls.input} mb-1.5`} />
            <select value={form.agentId} onChange={(e) => updateForm("agentId", e.target.value)} className={`w-full ${cls.select}`} size={Math.min(6, filteredAgents.length + 1)}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="text" value={form.areaLocation} onChange={(e) => updateForm("areaLocation", e.target.value)} className={cls.input} placeholder="e.g. Blue Area" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea value={form.propertyAddress} onChange={(e) => updateForm("propertyAddress", e.target.value)} rows={2} className={cls.textarea || cls.input} placeholder="Street, building name, landmarks" />
          </div>
          ...
          {/* Covered Area + Plot Size */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={cls.label}>Covered Area (sqft)</label>
              <input type="number" step="0.01" min="0" value={form.coveredAreaSqft} onChange={(e) => updateForm("coveredAreaSqft", e.target.value)} className={cls.input} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min="0" value={form.floors} onChange={(e) => updateForm("floors", e.target.value)} className={cls.input} />
            </div>
            <div>
              <label className={cls.label}>Floor Plate Size (sqft)</label>
              <input type="number" step="0.01" min="0" value={form.floorPlateSizeSqft} onChange={(e) => updateForm("floorPlateSizeSqft", e.target.value)} className={cls.input} />
            </div>
          </div>

          {/* Parking Capacity + Elevators */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={cls.label}>Parking Capacity</label>
              <input type="number" min="0" value={form.parkingCapacity} onChange={(e) => updateForm("parkingCapacity", e.target.value)} className={cls.input} />
            </div>
            <div>
              <label className={cls.label}>Elevators</label>
              <input type="number" min="0" value={form.elevators} onChange={(e) => updateForm("elevators", e.target.value)} className={cls.input} />
            </div>
          </div>

          {/* Building Status + Possession Timeline */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={cls.label}>Building Status</label>
              <select value={form.buildingStatus} onChange={(e) => updateForm("buildingStatus", e.target.value)} className={`w-full ${cls.select}`}>
                <option value="">—</option>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" step="0.01" min="0" value={form.askingRent} onChange={(e) => updateForm("askingRent", e.target.value)} className={cls.input} />
          ...
          <div className="flex justify-end gap-2 border-t border-gray-100 pt-3">
            <button onClick={() => navigate("/acquisitions/buildings")} className={cls.btnSecondary}>Cancel</button>
            <button onClick={handleSubmit} disabled={saving} className={cls.btnPrimary}>
              {saving ? "Saving…" : isEdit ? "Update Building" : "Create Building"}
            </button>
          </div>
        </div>
      </div>
